...
// Servlet 관련 코드를 작성하기 위해서는 HttpServlet 클래스 상속을 받아야 한다!
 ...
 * <servlet> <servlset-mapping>를 작성하는 효과
 */
...
public class ExampleController2 extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// -> JSP가 대신 화면을 만들어달라! (Servlst이 JSP에게 요청/응답을 위임)
		
		// JSP가 대신 응답 화면을 만들어주기 위해서는
		// Servlst이 어떤 요청을 받았는지 알아야 한다 ! (req, resp)
		
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/result.jsp");
																// webapp 폴더 기준
		// -> webapp 폴더는 웹 어플리케이션에 루트(root) 디렉토리로, 
		...
		// 지정한 JSP 페이지(result.sjp)로 전달하는 작업
